...
func main() {
	...
	newOrder.rcvClientCH = make(chan []byte, 1000)
	newOrder.ctrbcCH = make(chan []byte, 30000)
	newOrder.rbcCH = make(chan []byte, 30000)
	newOrder.baCH = make(chan []byte, 10000)
	newOrder.signal2tpCH = make(chan []byte, 10)
	newOrder.msgoutCH = make(chan pb.SendMsg, 10000)
	newOrder.conMsgBuff = make([]chan pb.SendMsg, newOrder.Node_num)
	for i := 0; i < newOrder.Node_num; i++ {
		newOrder.conMsgBuff[i] = make(chan pb.SendMsg, 3000)
	}
	...
	TpIPPath := fmt.Sprintf("%s%sip.txt", gopath, newOrder.TpIPPath)
